...
local ENEMY_DETECTION_RADIUS = 200 -- Max studs to detect enemies, keeps bot on patrol path
...
local function isFactoryOpen()
    ...
    -- 1. Check the BillboardGui on the Factory FrontDoor in Workspace
    local factorySign = Workspace:FindFirstChild("Factory") 
        and Workspace.Factory:FindFirstChild("FrontDoor") 
        and Workspace.Factory.FrontDoor:FindFirstChild("Top") 
        and Workspace.Factory.FrontDoor.Top:FindFirstChild("BillboardGui")
        and Workspace.Factory.FrontDoor.Top.BillboardGui:FindFirstChildOfClass("TextLabel")
        
    if factorySign then
        if string.find(string.upper(factorySign.Text), "FACTORY CLOSED") then
            isOpen = false
        end
    end
        ...
        for _, npc in pairs(allEnemies) do
            if isValidTarget(npc, targetName) then
                if rootPart and npc:FindFirstChild("HumanoidRootPart") then
                    local actualMyFloor = getFloorLevel(rootPart.Position.Y)
                    local enemyFloor = getFloorLevel(npc.HumanoidRootPart.Position.Y)
                    
                    -- ONLY target enemies on our CURRENT floor!
                    if actualMyFloor == enemyFloor then
                        -- Prevent the bot from flying UP to enemies and skipping the ramp!
                        -- If a normal enemy is more than 15 studs higher than us, ignore them for now.
                        local isTooHigh = (targetName ~= "FactoryPool" and (npc.HumanoidRootPart.Position.Y - rootPart.Position.Y) > 15)
                        
                        if not isTooHigh then
                            -- Prioritize horizontal distance (ignoring Y height differences)
                            ...
                            local dist = (flatRoot - flatEnemy).Magnitude
                            
                            if dist <= ENEMY_DETECTION_RADIUS and dist < closestDistance then
...
function Model.UpdateTracking(deltaTime)
    ...
    if targetSwitchTimer >= switchInterval then
        targetSwitchTimer = 0
        local oldEnemy = currentEnemy
        
        -- If we are climbing/descending the macro, lock the target so we don't get distracted
        -- and get stuck in endless route recalculation loops!
        if Model.State.routingToFloor ~= nil and currentEnemy and isValidTarget(currentEnemy, currentEnemy.Name) then
            -- Keep the exact same target!
        else
            currentEnemy = findBestTarget(allEnemies)
        end
        
        if currentEnemy and currentEnemy ~= oldEnemy then
            print("[AutoFarm] Detected " .. currentEnemy.Name .. ", attacking now!")
        end
    end
    ...
    if currentEnemy then
        -- Since findBestTarget ONLY returns enemies on the SAME floor now,
        -- we just fly straight to it and fight!
        Model.State.routingToFloor = nil
        ...
        -- Check if we fell, got lost, or just started patrol. 
        local currentWP = WAYPOINTS[Model.State.currentWaypointIndex]
        if not currentWP or (rootPart.Position - currentWP).Magnitude > 50 then
            -- Find the absolute beginning of the ramp for our CURRENT floor!
            local myFloor = getFloorLevel(rootPart.Position.Y)
            local startingIdx = 1
            
            for i, wp in ipairs(WAYPOINTS) do
                if getFloorLevel(wp.Y) == myFloor then
                    startingIdx = i
                    break
                end
            end
            
            Model.State.currentWaypointIndex = startingIdx
            Model.State.patrolDirection = 1 -- Always climb UP the macro!
        end
            ...
            if wallCast and wallCast.Distance <= 5 then
                ...
                for _, evasionDir in ipairs(EVASION_DIRECTIONS) do
                    ...
                    if not evadeCast then
                        print("[AutoFarm] Obstacle detected! Evading sideways...")
                ...
                if not evaded then
                    print("[AutoFarm] Obstacle detected! Evading upwards...")
                    Model.State.evadingTimer = 0.5
                    Model.State.evasionDir = Vector3.new(0, 1, 0)
    ...
    for currentHit = 1, 4 do
        ...
        for _, npc in pairs(currentTargets) do
            if npc.Name == "FactoryPool" then
                -- barrelHP is a ValueObject, so sending it causes the damage to fail. Send the hitbox Part instead!
                local hitbox = npc:FindFirstChild("hitbox")
                local hrp = npc:FindFirstChild("HumanoidRootPart")
                if hitbox then
                    table.insert(roots, hitbox)
                elseif hrp then
        ...
        if #roots > 0 then
            print("[AutoFarm] Sending damage packet for Hit " .. currentHit .. " to " .. #roots .. " targets!")
            local damageArgs = {
                [1] = {
                    [1] = "damage",
                    [2] = roots,
                    [3] = "Sword",
                    [4] = {[1] = currentHit, [2] = "Ground", [3] = "Sword"},
                    [5] = true,
                    [6] = myCFrame,
                    ["aircombo"] = "Ground"
                }
