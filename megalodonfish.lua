            ...
            while waited < maxWait do
                ...
                if hook:GetAttribute("Caught") == true or hook:FindFirstChild("ReelLoop") then
                    if Model.State.isDeepSeaCatcher then
                        ...
                        -- Target specifically the fast, relevant folders where active sounds can exist!
                        -- This completely avoids ReplicatedStorage (which is massive and causes stutters)
                        -- and avoids the massive static Map folders in Workspace.
                        local fastPlaces = {
                        ...
                        pcall(function()
                            for _, place in ipairs(fastPlaces) do
                                if place then
                                    for _, child in ipairs(place:GetDescendants()) do
                                        if child:IsA("Sound") and string.find(child.Name, "DeepSea") then
                                            table.insert(beastSounds, child)
                                        end
                                    end
                                end
                            end
                            
                            -- Also check direct children of workspace just in case (fast)
