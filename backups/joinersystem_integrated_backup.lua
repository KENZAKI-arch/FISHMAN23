...
function Model.UpdateCombatTracking(deltaTime)
    local character = LocalPlayer.Character
    ...
    task.spawn(function()
        RunService.Heartbeat:Connect(function(dt)
            ...
            if Model.State.isBatchAttacking or (Model.State.isDeepSeaCatcher or Model.State.isFishing) then
                pcall(function() Model.UpdateCombatTracking(dt) end)
            end
        end)
    end)
    ...
    task.spawn(function() while _running and task.wait() do if Model.State.isBatchAttacking then task.wait(0.5) continue end if Model.State.isFishing or Model.State.isDeepSeaCatcher then Model.DoFishingCycle() end end end)
