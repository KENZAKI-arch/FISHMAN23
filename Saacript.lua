...
function Model.UpdateTracking(deltaTime)
    ...
    if currentEnemy then
        ...
        -- Anti-stuck offset to shift forward/backward/sideways if missiles are hitting a wall
        if Model.State.antiStuckOffsetVector and Model.State.antiStuckOffsetVector.Magnitude > 0 then
            local right = Vector3.new(rootPart.CFrame.RightVector.X, 0, rootPart.CFrame.RightVector.Z).Unit
            local forward = Vector3.new(rootPart.CFrame.LookVector.X, 0, rootPart.CFrame.LookVector.Z).Unit
            
            local offsetX = Model.State.antiStuckOffsetVector.X
            local offsetZ = Model.State.antiStuckOffsetVector.Z
            
            -- x is Right/Left, -z is Forward
            local shift = (right * offsetX) + (forward * -offsetZ)
            targetSpot = targetSpot + shift
        end
                        ...
                        if isAtHoverSpot then -- Only bomb if we have arrived at the intended hover spot
                            ...
                            if skillName == "Missle Shower" then
                                ...
                                if releaseEvent then
                                    ...
                                    Model.State.antiStuckOffsetVector = Vector3.new(0, 0, 0)
                                    local lastHealth = -1
                                    local stuckTicks = 0
                                    local stuckState = 0
                                    
                                    local lastStamina = getStamina(LocalPlayer.Character)
                                    local ticksSinceLastShot = 999
                                    ...
                                    for i = 1, 110 do
                                        ...
                                        -- Track stamina drain to verify the skill is actively firing, not on cooldown
                                        local currentStamina = getStamina(LocalPlayer.Character)
                                        if currentStamina < lastStamina and lastStamina > 0 then
                                        ...
                                        local isActivelyFiring = (ticksSinceLastShot < 10) -- Considered firing if drained stamina within the last ~1 second
                                        ...
                                        if targetToShoot then
                                            local humanoid = targetToShoot:FindFirstChild("Humanoid")
                                            if humanoid then currentHealth = humanoid.Health end
                                            if targetToShoot.Name == "FactoryPool" then
                                                local barrelHP = targetToShoot:FindFirstChild("barrelHP")
                                                if barrelHP then currentHealth = barrelHP.Value end
                                            end
                                        end
                                        
                                        if lastHealth == -1 then
                                            lastHealth = currentHealth
                                        elseif currentHealth >= lastHealth and currentHealth > 0 then
                                            if isActivelyFiring then
                                                stuckTicks = stuckTicks + 1
                                                if stuckTicks >= 20 then -- ~1.8 seconds of actively firing but no damage
                                                    stuckState = (stuckState % 4) + 1
                                                    if stuckState == 1 then
                                                        Model.State.antiStuckOffsetVector = Vector3.new(0, 0, -10) -- Forward
                                                    elseif stuckState == 2 then
                                                        Model.State.antiStuckOffsetVector = Vector3.new(0, 0, 10)  -- Backward
                                                    elseif stuckState == 3 then
                                                        Model.State.antiStuckOffsetVector = Vector3.new(10, 0, 0)  -- Right
                                                    elseif stuckState == 4 then
                                                        Model.State.antiStuckOffsetVector = Vector3.new(-10, 0, 0) -- Left
                                                    end
                                                    stuckTicks = 0
                                                end
                                            end
