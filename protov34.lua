...
function Model.UpdateTracking(deltaTime)
    ...
    if not Model.State.hasEnteredFactory then
        ...
        if factory then
            ...
            -- The factory entrance is around X=8804, Z=11617. Inside is smaller X and larger Z.
            if pPos.X < 8795 and pPos.Z > 11620 then
                isInside = true
            end
        end
        ...
        for _, tName in ipairs(TARGET_SEQUENCE) do
            if isValidTarget(npc, tName) then
                ...
                if root then
                    ...
                    local alt = getgenv().CyborgFlyAltitude or 190
                    if npc.Name == "Law" then alt = 400 end
                                ...
                                if releaseEvent then
                                    ...
                                    for i = 1, 110 do
                                        ...
                                        if targetToShoot and targetToShoot.Name == "FactoryPool" then
                                            ...
                                            -- Aim straight down from the player's current X and Z to avoid clipping the walls of the factory access hole
                                            currentAim = CFrame.new(Vector3.new(LocalPlayer.Character.HumanoidRootPart.Position.X, hPos.Y, LocalPlayer.Character.HumanoidRootPart.Position.Z))
