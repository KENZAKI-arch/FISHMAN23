            ...
            while waited < maxWait do
                ...
                if hook:GetAttribute("Caught") == true or hook:FindFirstChild("ReelLoop") then
                    if Model.State.isDeepSeaCatcher then
                        ...
                        -- Find all matching sounds anywhere in the game to see where it's hiding
                            ...
                            if place then
                                pcall(function()
                                    for _, child in ipairs(place:GetDescendants()) do
                                        if child:IsA("Sound") and (child.Name == "Small" or child.Name == "Medium" or child.Name == "Large" or child.Name == "Giant" or string.find(child.Name, "DeepSea")) then
                                            table.insert(soundsToCheck, child)
                                        end
                                    end
                                end)
                            end
                        ...
                        print("🔎 Found", #soundsToCheck, "matching sounds in the game. Monitoring them for 3 seconds...")
                        
                        while _running and bWaited < maxWait and hook.Parent do
                            for _, child in ipairs(soundsToCheck) do
                                local state = tostring(child.Playing) .. "_" .. tostring(child.Volume) .. "_" .. tostring(child.TimePosition > 0)
                                if printedSounds[child] ~= state then
                                    print("🔊 SOUND:", child.Name, "| Playing:", child.Playing, "| Vol:", child.Volume, "| TimePos:", child.TimePosition, "| Parent:", child.Parent and child.Parent.Name or "nil")
                                    printedSounds[child] = state
                                end
                            end
                            task.wait(0.1)
                            bWaited += 0.1
...
-- ======================================================================
    ...
    Tabs.Fishing:AddSlider("S_DeepSeaWait", { Title = "Megalodon Bite Delay (Sec)", Default = 3, Min = 0, Max = 10, Rounding = 1, Callback = function(Value)
        Model.State.deepSeaWait = Value
    end })
    Tabs.Fishing:AddSlider("S_DeepSeaTimeout", { Title = "Megalodon Spawn Timeout (Sec)", Default = 15, Min = 5, Max = 45, Rounding = 1, Callback = function(Value)
        Model.State.deepSeaTimeout = Value
    end })
