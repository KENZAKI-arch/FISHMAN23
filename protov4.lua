...
local function getAllEnemies()
    ...
    -- Explicitly add FactoryPool since it's nested deep in the Islands folder
    ...
    if factory then
        local fp = factory:FindFirstChild("FactoryPool")
        if fp then
            table.insert(allEnemies, fp)
        end
    end
...
local function getRoot(npc)
    ...
    if npc.Name == "FactoryPool" then
        return npc:FindFirstChild("hitbox") or npc:FindFirstChild("Pool") or npc:FindFirstChildWhichIsA("BasePart")
    end
...
local function isValidTarget(npc, targetName)
    ...
    if npc.Name == "FactoryPool" then
        ...
        if barrelHP then
            return barrelHP.Value > 0
        elseif humanoid then
            return humanoid.Health > 0
        else
            if npc:FindFirstChild("hitbox") or npc:FindFirstChild("Pool") then return true end
            return false
        end
    end
...
local function findBestTarget(allEnemies)
    ...
    if #validEnemies > 0 and rootPart then
        ...
        table.sort(validEnemies, function(a, b)
            ...
            if a.seq ~= b.seq then
                return a.seq < b.seq -- Absolute Priority 1: Target Sequence (Scientist MUST be killed before FactoryPool)
            elseif levelA ~= levelB then
                return levelA < levelB -- Priority 2: Lower floors first
            else
...
function Model.UpdateTracking(deltaTime)
    ...
    if isFactoryClosedOrHighAlert then
        ...
        local safeSpot = Vector3.new(8812, 66, 11514)
        ...
        if (flatTarget - flatCurrent).Magnitude < 15 then
            ...
            -- No flyMode check needed, use physics!
    ...
    elseif Model.State.wasWaitingForFactory then
        ...
        Model.State.hasEnteredFactory = false -- Reset entry state so it repeats the startup route!
        Model.State.resumePatrolTime = os.clock() + 10
        print("[AutoFarm] Factory/Alert cleared! Waiting 10 seconds for the doors to fully open...")
        Model.State.isWaitingAtSafeSpot = true
        Model.SetFlightAnimation(false)
        local bv = rootPart:FindFirstChild("AntiGravity")
        if bv then bv:Destroy() end
    ...
    if not Model.State.hasEnteredFactory then
        local isInside = false
        local islands = workspace:FindFirstChild("Islands")
        local factory = islands and islands:FindFirstChild("Rose Kingdom") and islands["Rose Kingdom"]:FindFirstChild("Factory")
        
        if factory then
            local pPos = rootPart.Position
            -- If we are on the upper floors (Law is ~280), we are definitely inside!
            if pPos.Y > 150 then
                isInside = true
            -- If we are on the ground floor, check if we are past the entrance (X < 8795) and within the factory width
            elseif pPos.X < 8795 and pPos.Z > 11400 and pPos.Z < 12500 then
                isInside = true
            end
        end

        if isInside then
            Model.State.hasEnteredFactory = true
        else
            local entrySpot = Vector3.new(8804, 66, 11617)
            local flatTarget = Vector3.new(entrySpot.X, 0, entrySpot.Z)
            local flatCurrent = Vector3.new(rootPart.Position.X, 0, rootPart.Position.Z)
            ...
            if (flatTarget - flatCurrent).Magnitude > 15 then
                ...
                -- Fly directly to the entry spot in 3D (diagonally down) so we don't hit the wall above the entrance
                local moveSpot = entrySpot
                local direction = (moveSpot - rootPart.Position).Unit
    ...
    if currentEnemy then
        ...
        if currentEnemy.Name == "FactoryPool" then
            ...
            targetSpot = Vector3.new(8664, 650, 11793)
        elseif targetRoot.Position.Y >= 267 and rootPart.Position.Y < 275 then
            targetSpot = Vector3.new(8777, 295, 11826)
        end
        ...
        if targetSpot == Vector3.new(8777, 295, 11826) then
            -- The access point is a wall! Noclip DIRECTLY through it instead of going straight up first.
            moveTargetSpot = targetSpot
        elseif Model.State.hasEnteredFactory and targetSpot.Y > rootPart.Position.Y + 30 then
            -- Inside factory: Generic ascend logic (Y first) to cleanly navigate multi-story floors.
            moveTargetSpot = Vector3.new(rootPart.Position.X, targetSpot.Y, rootPart.Position.Z)
        elseif not Model.State.hasEnteredFactory and Vector2.new(rootPart.Position.X - targetSpot.X, rootPart.Position.Z - targetSpot.Z).Magnitude > 30 then
            -- Outside factory: Head in X and Z first! Maintain current Y, then drop/ascend when horizontally close.
            moveTargetSpot = Vector3.new(targetSpot.X, rootPart.Position.Y, targetSpot.Z)
        end
        ...
        for _, tName in ipairs(TARGET_SEQUENCE) do
            if isValidTarget(npc, tName) then
                ...
                if root then
                    ...
                    local alt = getgenv().CyborgFlyAltitude or 190
                    if npc.Name == "Law" then alt = 310 end
                    local expectedY = myPos.Y - alt
                    ...
                    for _, skillName in ipairs(skills) do
                        ...
                        if isAtHoverSpot then -- Only bomb if we have arrived at the intended hover spot
                            ...
                            pcall(function()
                                ...
                                elseif currentEnemy.Name == "FactoryPool" then
                                    local hitbox = currentEnemy:FindFirstChild("hitbox") or currentEnemy:FindFirstChild("Pool") or currentEnemy:FindFirstChildWhichIsA("BasePart")
                                    if hitbox then 
                                        -- Aim straight down from the hover spot so we don't clip the walls of the chimney/hole
                                        targetPos = Vector3.new(rootPart.Position.X, hitbox.Position.Y, rootPart.Position.Z)
                                    end
                                end
                            ...
                            if skillName == "Missle Shower" then
                                ...
                                if releaseEvent then
                                    ...
                                    for i = 1, 110 do
                                        ...
                                        -- ABSOLUTE PRIORITY: If we started shooting the FactoryPool, but a Scientist spawned, STOP IMMEDIATELY!
                                        if initialTarget and initialTarget.Name == "FactoryPool" then
                                            if currentEnemy and (currentEnemy.Name == "Scientist" or currentEnemy.Name == "Devil Fruit Scientist" or currentEnemy.Name == "Law") then
                                                break -- Cancel Missle Shower to go hunt the newly spawned Scientist!
                                            end
                                        end
                                        ...
                                        if targetToShoot and targetToShoot.Name == "FactoryPool" then
                                            local hitbox = targetToShoot:FindFirstChild("hitbox") or targetToShoot:FindFirstChild("Pool") or targetToShoot:FindFirstChildWhichIsA("BasePart")
                                            local hPos = hitbox and hitbox.Position or (LocalPlayer.Character.HumanoidRootPart.Position - Vector3.new(0, 50, 0))
                                            
                                            local myPos = LocalPlayer.Character.HumanoidRootPart.Position
                                            local hoverXZ = Vector2.new(8664, 11793)
                                            local myXZ = Vector2.new(myPos.X, myPos.Z)
                                            
                                            if (myXZ - hoverXZ).Magnitude < 20 then
                                                -- Aim straight down to avoid clipping chimney walls when hovering above
                                                currentAim = CFrame.new(Vector3.new(myPos.X, hPos.Y, myPos.Z))
                                            else
                                                -- Aim directly at the pool when flying towards it from inside the factory
                                                currentAim = CFrame.new(hPos)
                                            end
