...
local function RunCraftingSequence()
...
local function ToggleScript()
    ...
    if isRunning then
        toggleBtn.Text = "STOP CRAFTING"
        toggleBtn.BackgroundColor3 = Color3.fromRGB(255, 85, 85)
        task.spawn(RunCraftingSequence)
    else
        toggleBtn.Text = "START CRAFTING"
        toggleBtn.BackgroundColor3 = Color3.fromRGB(85, 255, 85)
...
stopConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode.RightBracket then
        if isRunning then ToggleScript() end
    end
end)
