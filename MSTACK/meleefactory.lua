...
function Model.UpdateTracking(deltaTime)
    ...
    elseif Model.State.isGateWaiting then
        ...
        if bv then bv:Destroy() end
        return
    elseif Model.State.botMode == "PATROL" then
        currentEnemy = nil
        isPatrolling = true
    ...
    elseif Model.State.botMode == "COMBAT" then
        if not currentEnemy then
            ...
            local launchFloor = getFloorLevel(WAYPOINTS[Model.State.currentWaypointIndex].Y)
            local hrp = currentEnemy:FindFirstChild("HumanoidRootPart")
            local enemyFloor = launchFloor
            if hrp then
                enemyFloor = getFloorLevel(hrp.Position.Y)
            ...
            if launchFloor ~= enemyFloor then
                -- Enemy fell off! Go back to PATROL to walk the stairs to them.
                Model.State.botMode = "PATROL"
                currentEnemy = nil
                isPatrolling = true
                Model.State.patrolDirection = (enemyFloor > launchFloor) and 1 or -1
                targetDest = WAYPOINTS[Model.State.currentWaypointIndex] or rootPart.Position
            else
...
function Model.DoMeleeCombo()
    ...
    print("[AutoFarm] Enemies in range! Starting Melee Combo...")
...
function View.Build(onToggleCallback)
    ...
    toggleBtn.BackgroundColor3 = Color3.fromRGB(255, 85, 85) -- Red Color for Melee!
    toggleBtn.Text = "MELEE FARM: OFF"
    ...
    local function setFarmingState(state)
        ...
        toggleBtn.Text = isFarming and "MELEE FARM: ON" or "MELEE FARM: OFF"
